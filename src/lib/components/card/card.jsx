...
import { Button } from "../button/Button.jsx"; // Sesuaikan path import ini
...
export function Card({
  ...
  const config = {
    absen_belum: {
      // Menggunakan --color-danger (#ff8b5c - Orange)
      bg: "bg-danger", 
      title: "Anda belum absen hari ini!",
      subTitle: "Absensi anda",
      btnText: "Clock In",
      // Menggunakan varian 'clock' dari komponen Button kamu (bg-brand-400)
      btnVariant: "clock", 
      isCuti: false,
    },
    absen_sudah: {
      // Menggunakan --color-brand (#9bb7cd - Blue/Gray)
      bg: "bg-brand", 
    ...
    absen_lengkap: {
      // Menggunakan --color-success (#65d148 - Green)
      bg: "bg-success", 
    ...
    cuti: {
      // Menggunakan --color-info (#7381fc - Purple/Indigo)
      bg: "bg-info", 
      title: "Info cuti anda",
      subTitle: null,
      btnText: "Ambil cuti",
      // Varian clock cocok karena warnanya soft (brand-400)
      btnVariant: "clock", 
      isCuti: true,
    },
  };
  ...
  return (
    <div className="w-97 max-w-sm bg-white rounded-xl shadow-md overflow-hidden font-sans border flex flex-col h-fit">
      ...
      <div className={`${style.bg} p-5 text-white`}>
        ...
        {style.subTitle && <p className="text-xs opacity-90 mt-1">{style.subTitle}</p>}
      </div>
        ...
        {style.isCuti ? (
          <div className="flex flex-col h-full space-y-6">
            <div className="text-center space-y-6 text-brand-900 mt-2">
              <div>
                <p className="text-xs text-brand-700 mb-1">Total pengajuan cuti tahun ini</p>
                <p className="text-5xl font-bold">{totalCuti || 0}</p>
              </div>
              <div className="border-t border-brand-200 pt-4">
                <p className="text-xs text-brand-700 mb-1">Cuti tersisa tahun ini</p>
          ...
          <div className="flex flex-row h-full gap-4">
            ...
            <div className="flex-1 flex flex-col gap-5 text-brand-900">
              ...
              <div>
                ...
                <p className={`font-semibold ${jamMasuk ? "text-3xl" : "text-xl"}`}>
                  {jamMasuk || "-"}
                </p>
              </div>
              ...
              <div>
                ...
                <p className={`font-semibold ${jamPulang ? "text-3xl" : "text-xl"}`}>
                  {jamPulang || "-"}
                </p>
              </div>
            ...
            <div className="w-[40%] flex flex-col justify-end items-end">
              {variant === "absen_lengkap" ? (
                // Tombol Disabled (Manual style menggunakan var --color-disable-color)
